...
class TestRainfall(unittest.TestCase):
    ''' rainfall (16) kyu6'''
    test_cases = {"data": """Rome:Jan 81.2,Feb 63.2,Mar 70.3,Apr 55.7,May 53.0,Jun 36.4,Jul 17.5,Aug 27.5,Sep 60.9,Oct 117.7,Nov 111.0,Dec 97.9\n
...
Lima:Jan 11.2,Feb 10.9,Mar 10.7,Apr 10.4,May 10.6,Jun 11.8,Jul 14.4,Aug 13.1,Sep 23.3,Oct 1.7,Nov 0.5,Dec 10.7"""
    ...
    def test_rainfall(self):
        for name, kyu in IMPLEMENTATIONS.items():
            print u"Tests for Rainfall: {}".format(name)
            self.assertEqual(kyu["kyu6"].mean(town="London", strng=self.test_cases["data"]), 51.199999999999996)
            self.assertEqual(kyu["kyu6"].mean(town="Beijing", strng=self.test_cases["data"]), 52.416666666666664)

            self.assertEqual(kyu["kyu6"].variance("London", strng=self.test_cases["data"]), 57.428333333333335)
            self.assertEqual(kyu["kyu6"].variance("Beijing", strng=self.test_cases["data"]), 4808.37138888889)

            self.assertEqual(kyu["kyu6"].mean("London", strng=self.test_cases["data1"]), 57.03333333333333)
            self.assertEqual(kyu["kyu6"].mean("Beijing", strng=self.test_cases["data1"]), 59.083333333333336)

            self.assertEqual(kyu["kyu6"].variance("London", strng=self.test_cases["data1"]), 110.90055555555556)
            self.assertEqual(kyu["kyu6"].variance("Beijing", strng=self.test_cases["data1"]), 4437.038055555556)
